...
public final class PGNParser extends Analizador implements PGNParserConstants {
	...
	final public void PGNTag () throws ParseException {
		...
		if (imageOf (t).equals ("Event")) {
			...
			if (imageOf (t).equals ("Site")) {
				partida.sitio = st;
			}
			else
				if (imageOf (t).equals ("Round")) {
					partida.ronda = st;
				}
				else
					if (imageOf (t).equals ("Result")) {
						partida.resultado = st;
					}
					else
						if (imageOf (t).equals ("Date")) {
							partida.fecha = st;
						}
						else
							if (imageOf (t).equals ("White")) {
								partida.jugadorB = st;
							}
							else
								if (imageOf (t).equals ("Black")) {
									partida.jugadorN = st;
								}
								else
									if (imageOf (t).equals ("ECO")) {
										partida.ECO = st;
									}
									else
										if (imageOf (t).equals ("TimeControl")) {
		...
		if (jj_2_3 (2)) {
			...
			switch ((jj_ntk == -1) ? jj_ntk () : jj_ntk) {
				...
				case SQUARE:
					 ...
					 */
					partida.listaMovimientos.append (imageOf (ff)
							+ imageOf (aa) + imageOf (ss) + imageOf (pp)
							+ imageOf (ii) + " ");
					break;
				default:
					jj_la1[18] = jj_gen;
					if (jj_2_4 (3)) {
						...
						partida.listaMovimientos.append (imageOf (ii)
								+ imageOf (ff) + imageOf (aa) + imageOf (pp)
								+ imageOf (ss) + " ");
					}
					else {
						switch ((jj_ntk == -1) ? jj_ntk () : jj_ntk) {
							case PIECE_IDENT:
								 ...
								 * if(metaDataOnly) return null;
								 * token.setPiece(imageOf(ii));
								 * token.setFrom(imageOf(ff));
								 * token.setTo(imageOf(ss));
								 * token.setCapture(imageOf(aa)); return token;
								 */
								partida.listaMovimientos.append (imageOf (ii)
										+ imageOf (ff) + imageOf (rr)
										+ imageOf (aa) + imageOf (ss) + " ");
								break;
	...
	final public void MoveText () throws ParseException {
		 ...
		 */
		partida.listaMovimientos.deleteCharAt (partida.listaMovimientos
				.length () - 1);
	...
	public static void guardarPartida (String archivo, Partida p) throws IOException {
		java.io.FileWriter f = new java.io.FileWriter (archivo, true);
		
		/*
		 * Los siete campos obligatorios son: Event Site Date Round White
		 * Black Result
		 */
		if (p.evento != null)
